...
const getBlockByType = async () => {
    try {
        ...
        blocks.value = response.data.data.filter((block: Block) => !props.pageForm.blocks.some((pageBlock) => pageBlock.id === block.id));
    } catch (error) {
        console.error(error);
    }
};
...
<template>
    <div class="flex flex-col gap-4">
        <Action tag="button" variant="outline" v-if="currentStep > 1" @click="previous" class="self-start">← Previous</Action>

        <!-- Step 1 -->
        <template v-if="currentStep === 1">
            <BlockTypeSelector :block-types="blockTypes" v-model:current-step="currentStep" v-model:type="type" />
        </template>

        <!-- Step 2 -->
        <template v-if="currentStep === 2">
            <template v-if="blocks.length > 0">
                <div class="overflow-overlay relative rounded-lg">
                    <table class="w-full border-separate border-spacing-0">
                        <thead class="bg-secondary-950 text-white">
                            <tr>
                                <th class="px-4 py-2.5 text-left">Select</th>
                                <th v-for="col in tableCol" :key="col" class="px-4 py-2.5 text-left">{{ col }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr
                                ...
                                :key="block.id"
                                class="border-secondary-200 hover:bg-secondary-100 odd:bg-secondary-50 relative border-b transition"
                            >
        ...
        <Action tag="button" variant="primary" v-if="currentStep === totalStep && blocks.length > 0" @click="pushBlocks">
            Add {{ blocksToPush.length }} Block(s)
        </Action>
        <Action tag="button" variant="primary" :disabled="type === ''" v-if="currentStep !== totalStep" @click="next" class="self-end">Next →</Action>
    </div>
</template>
