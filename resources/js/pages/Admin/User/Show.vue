...
<template>
    <PanelLayout :title="user.name">
        ...
        <template #action>
            ...
            <Can permission="delete users">
                <Action
                    tag="button"
                    :variant="['delete', 'icon']"
                    @click="deleteItem(route('admin.user.destroy', user.id), 'Are you sure you want to delete this user?')"
                >
                    <IconTrash />
                </Action>
            </Can>
        ...
        <section class="flex flex-col gap-4">
            ...
            <InputLabel label="Email" name="email" v-model="form.email" disabled hint="You can't modify email address" :error="form.errors.email" />
        </section>
            ...
            <section class="mt-8 flex flex-col gap-4">
                ...
                <div class="overflow-hidden rounded-lg">
                    <table class="w-full border-separate border-spacing-0">
                        ...
                        <tbody>
                            <tr v-for="action in actions" :key="action" class="border-secondary-200 odd:bg-secondary-50 relative border-b transition">
