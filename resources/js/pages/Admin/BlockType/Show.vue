    ...
    <PanelLayout :title="blockType.name">
        <template #action>
            <Can permission="create blocks">
                <Action tag="Link" variant="primary" size="normal" :href="route('admin.block.create', blockType.type)">
                    <IconPlus />
        ...
        <section class="mb-6">
            ...
            <dl class="flex flex-col gap-6">
                <div class="flex gap-12">
                    ...
                    <div class="flex flex-col">
                        <dt class="text-lg font-medium">Type</dt>
                        <dd>{{ blockType.type }}</dd>
                    </div>
                </div>
                <div class="flex flex-col gap-2">
                    <dt class="text-lg font-medium">Fields</dt>
                    <ul class="flex flex-col gap-2">
                        <Container tag="li" v-for="(field, index) in blockType.fields" :key="index" class="flex items-center gap-2 font-mono">
                            {{ field.name }}
                        </Container>
                    </ul>
                </div>
        ...
        <section>
            <h3 class="mb-2 text-xl font-medium">Blocks</h3>
            <Table
