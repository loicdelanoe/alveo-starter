    ...
    <PanelLayout :title="block.block_type.name">
        <template #action>
            <Can permission="edit blocks">
                ...
                <Action tag="button" variant="primary" @click.prevent="form.patch(route('admin.block.update', props.block.id))">
                    <IconCheck />
                    Save
                </Action>
            </Can>
            <Can permission="delete blocks">
                <Action
                    tag="button"
                    :variant="['delete', 'icon']"
                    @click.prevent="deleteItem(route('admin.block.destroy', props.block.id), 'Are you sure you want to delete this block?', form)"
                >
                    <IconTrash />
                </Action>
            </Can>
        ...
        <!-- Informations -->
        <section class="mb-12">
            <h3 class="mb-2 text-2xl">Informations</h3>
            <p>This block is present in {{ block.pages.length }} page(s) :</p>
            <ul class="flex flex-col gap-2">
                <li v-for="page in block.pages" :key="page.slug" class="border-secondary-200 flex items-center justify-between border-b py-2">
                    <span class="font-medium">{{ page.title }}</span>
                    <div class="flex gap-4">
                        <VisitLink :href="route('page.show', page.slug)" :is-home="page.is_home" />
                        <Can permission="edit pages">
                            <Action tag="Link" variant="outline" size="small" :href="route('admin.page.show', page.slug)"> Edit Page </Action>
                        </Can>
                    </div>
                </li>
            </ul>
        ...
        <section class="flex flex-col gap-4">
            ...
            <Container tag="form" class="flex flex-col gap-4">
                <div v-for="field in block.block_type.fields" class="flex flex-col gap-2" :key="JSON.stringify(field)">
